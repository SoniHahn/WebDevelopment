...
var drinkType = null;
var inebriationnn = null;

// function selectType(){
// 	console.log($(this).val())
// }

var drinktypes = document.getElementsByClassName('drinkType');

for (var i = 0; i < drinktypes.length; i++){

	drinktypes[i].addEventListener('click', function(){
		console.log(this)
		drinkType = $(this).val();
...
var inebriations = document.getElementsByClassName('inebriationClass');

for (var i = 0; i < inebriations.length; i++){

	inebriations[i].addEventListener('click', function(){
		console.log(this)
		inebriationnn = $(this).val();
...
jQuery("#myForm").submit(function(e){
	...
	var dollarspent = jQuery("#dollarspent").val();
	var timespent = jQuery("#timespent").val();
	var occasion = jQuery("#occasion").val();
	var location = jQuery("#drinkLocation").val();
	var drinkDate = jQuery("#drinkDate").val();
	var inebriation = inebriationnn;
    var drinkLocation = jQuery("#drinkLocation").val();
	...
	jQuery.ajax({
  	...
  	success : function(response){
  		if(response.status=="OK"){
	  		...
	  		jQuery("#myForm input").val('');
	  		window.location.replace("/drinks-diary");
	  	}
	  	else {
	...
	jQuery.ajax({
		...
		success : function(response) {
			...
			for(var i=0;i<drinks.length;i++){
				...
				var latLng = {
					lat: drinks[i].location.geo[1], 
					lng: drinks[i].location.geo[0]
				}
				...
				var marker = new google.maps.Marker({
					map: map,
					position: latLng,
					title : drinks[i].drinkLocation + "<br>" + drinks[i].location.name
				});

				bindInfoWindow(marker, map, infowindow, '<b>'+drinks[i].drinkLocation + "<br>" + animals[i].location.name);
...
function renderDrinks(drinks){
	...
	// 	'<img class="url" src="'+drinks[i].url+'">'+
